...
// This function is responsible for fetching a single product based on its ID.
exports.getProduct = (req, res, next) => {
  // Retrieve the product ID from the request parameters
  Product.findById(req.params.productId)
    .then((product) => {
      // If the product is found, render the product-detail view, passing the product data
      res.render("shop/product-detail", {
        product: product, // The product data to be displayed
        pageTitle: product.title, // Set the page title to the product's title
        path: "/products", // Set the path to highlight the correct menu item
      });
    })
    .catch((err) => {
